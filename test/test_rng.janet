...
(let [rng-ori (assert (rng/new))
      ...
      seed (range 8)]
  (assert (not= (:get rng-ori 30) (:get rng-src 30)))
...
(let [seed (string/from-bytes ;(range 64))
  ...
  (assert (= (:get drbg1 32) (:get drbg2 32)))
  (assert (= (:get drbg1 64) (:get drbg2 64))))
...
(let [seed-a (string/from-bytes ;(range 64))
  ...
  (assert (not= (:get drbg-a 32) (:get drbg-b 32))))
...
(let [seed (string/from-bytes ;(range 64))
  ...
  (assert (= (:get-with-input drbg1 32 addl)
...
(let [seed (string/from-bytes ;(range 64))
      ...
      drbg1 (rng/new-drbg "HMAC_DRBG(SHA-256)" seed)
      drbg2 (rng/new-drbg "HMAC_DRBG(SHA-256)" seed)]
  (assert (not= (:get-with-input drbg1 32 addl-a)
